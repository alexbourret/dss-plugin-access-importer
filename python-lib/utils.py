# From access_parser by Uri Katz https://github.com/claroty/access_parser/blob/master/access_parser/utils.py
...
def read_db_file(db_path_or_handle):
    if hasattr(db_path_or_handle, 'read') and callable(db_path_or_handle.read):
        return db_path_or_handle.read()
    if not os.path.isfile(db_path_or_handle):
        logging.error(f"File {db_path_or_handle} not found")
        raise FileNotFoundError(f"File {db_path_or_handle} not found")
    with open(db_path_or_handle, "rb") as file_handle:
