# From access_parser by Uri Katz https://github.com/claroty/access_parser/blob/master/access_parser/access_parser.py
...
class AccessParser(object):
    def __init__(self, db_path_or_handle):
        self.db_data = read_db_file(db_path_or_handle)
...
class AccessTable(object):
    ...
    def _parse_row(self, record):
        ...
        if null_table_len and null_table_len < len(original_record):
            ...
            return
        if self.version > 3:
            _ = struct.unpack_from("h", record)[0]
            record = record[2:]
        else:
            _ = struct.unpack_from("b", record)[0]
            record = record[1:]
    ...
    def _parse_dynamic_length_records_metadata(self, reverse_record, original_record, null_table_length):
        ...
        if relative_record_metadata and \
            ...
            if metadata_start != -1 and metadata_start < 10:
                ...
                relative_record_metadata.relative_metadata_end = relative_record_metadata.relative_metadata_end + metadata_start
            else:
                logging.warning(
